...
type KaryawanSidebarProps = {
  navItems: Array<{ href: string; label: string }>;
};

export function KaryawanSidebar({ navItems }: KaryawanSidebarProps) {
  ...
  return (
    <aside className={`${isOpen ? "w-60" : "w-14"} shrink-0 border-r border-[#e5e7eb] bg-[#111111] text-[#9ca3af] transition-[width] duration-200`}>
      <div className="flex items-center justify-between border-b border-[#2a2a2a] px-3 py-3">
        {isOpen && <span className="text-xs font-semibold uppercase tracking-[0.08em] text-white">Panel Karyawan</span>}
        <button
          type="button"
